...
docker run -it --rm \
    ...
    -v ./data:/data \
    -v ./config:/config \
    ghcr.io/42bytelabs/konarr:v0.4.0
```
...
- `ghcr.io/42bytelabs/konarr:v0.4.0` - This is the name of the Docker image that we want to run. The `ghcr.io/42bytelabs/konarr` part is the name of the repository on GitHub Container Registry, and the `v0.1.0` part is the tag of the image that we want to run.
...
docker run -it --rm \
    ...
    -e KONARR_PROJECT_ID \
    ghcr.io/42bytelabs/konarr-agent:v0.4.0
```
